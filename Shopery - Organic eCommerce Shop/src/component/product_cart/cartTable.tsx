...
const CartTable = () => {
  ...
  <section className="grid grid-cols-3 max-2xl:grid-cols-1 gap-5 max-2xl:p-2">
        ...
        <tbody className="p-4">
          {cart.map((item) => (
            <tr key={item.id} className="border-b">
              <td className="flex items-center space-x-4 py-4 max-sm:space-x-0">
                <img
                  src={item.image}
                  alt={item.name}
                  className="h-16 w-16 object-cover max-sm:hidden"
                />
                <span className="max-sm:flex max-sm:flex-wrap">{item.name}</span>
              </td>
              <td className="py-4">${item.price.toFixed(2)}</td>
              <td className="py-4">
                <div className="flex items-center space-x-2 border-[1px] rounded-full p-2 w-[60%] max-md:border-0 max-sm:space-x-0">
                  <button
                    onClick={() => updateCartQuantity(item.id, -1)}
                    className="rounded-full border w-8 h-8 max-md:border-0"
                  >
                    -
                  </button>
    ...
    <div className="w-[424px] h-[276px]  border-[1px] rounded-md p-6 max-2xl:m-auto max-sm:w-[370px]">
